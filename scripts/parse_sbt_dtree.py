...
def remove_lines_before_graph(infile):
    ...
    for line in infile:
        ...
        if '[info]' not in line or '+-' not in line or line.strip().endswith('|') or \
                'evicted' in line:
            continue
        yield line
...
def main(mvn_dtree_file_path):
    ...
    with open('dtree-sbt.csv', 'wt', encoding='utf-8') as outfile:
...
if __name__ == '__main__':
    import sys
    if len(sys.argv) != 2:
        print(f'Usage: {__file__} <sbt-generated-dependency-tree>')
        exit()
